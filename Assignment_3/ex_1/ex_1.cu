...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    unsigned int tid=ix+iy*blockDim.x*gridDim.x;
    if(tid>width*height)
        return; 

    image_out[tid] = image[tid*3]*0.0722 + image[tid*3+1]*0.7152 + image[tid*3+2]*0.2126;
}
...
inline __device__ void copy_submatrix(float* in, int width, int height, float* out, int out_dim){
    int lx = threadIdx.x; 
    int ly = threadIdx.y; 

    int t_x = blockIdx.x * blockDim.x ;
    int t_y = blockIdx.y * blockDim.y ; 

    for(int l_row = ly; l_row<out_dim; l_row+=blockDim.y){
        for(int l_col = lx; l_col<out_dim; l_col+=blockDim.x){
            int y = t_y+l_row; 
            int x = t_x+l_col; 
            if(y<height && x<width){
                out[l_row*out_dim + l_col] = in[(t_y+l_row)*width + t_x+l_col];         
            }
        }
    }
}
 ...
 */
inline __device__ float gpu_applyFilter(float *image, int stride, float *matrix, int filter_dim)
{
    ////////////////
    // TO-DO #5.2 ////////////////////////////////////////////////
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        image_out[offset + 1] = gpu_applyFilter(&image[offset_t],
                                            width, gaussian, 3);
    }
}
__global__ void gpu_gaussian_cached(int width, int height, float *image, float *image_out)
{
    float gaussian[9] = { 1.0f / 16.0f, 2.0f / 16.0f, 1.0f / 16.0f,
                          2.0f / 16.0f, 4.0f / 16.0f, 2.0f / 16.0f,
                          1.0f / 16.0f, 2.0f / 16.0f, 1.0f / 16.0f };
    ...
    int offset   = (iy + 1) * width + ix;    
    int local_offset_t  =  threadIdx.y*BLOCK_SIZE_SH + threadIdx.x; 

    __shared__ float sh_block[BLOCK_SIZE_SH * BLOCK_SIZE_SH];
    copy_submatrix(image, width, height, sh_block, BLOCK_SIZE_SH); 
    __syncthreads();

    if (ix < (width - 2) && iy < (height - 2))
    {
        image_out[offset+1] = gpu_applyFilter(&sh_block[local_offset_t],
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    int offset_t = iy * width;
    int offset   = (iy + 1) * width;
    ...
    if (iy < (height - 2))
    {
        if (ix < (width - 2))
        {
            ...
            float gx = gpu_applyFilter(&image[offset_t + ix], width, sobel_x, 3);
            float gy = gpu_applyFilter(&image[offset_t + ix], width, sobel_y, 3);
            
            // Note: The output can be negative or exceed the max. color value
            // of 255. We compensate this afterwards while storing the file.
            image_out[offset + (ix + 1)] = sqrtf(gx * gx + gy * gy);
        }
    }

}
__global__ void gpu_sobel_cached(int width, int height, float *image, float *image_out)
{
    ////////////////
    // TO-DO #6.1 /////////////////////////////////////
    // Implement the GPU version of the Sobel filter //
    ...
    unsigned int tid=ix+iy*blockDim.x*gridDim.x;
    if(tid>width*height){
        printf("skipping tid: %d\n", tid); 
        return; 
    }

    //int offset_t = iy * width + ix;
    int offset   = (iy + 1) * width + (ix + 1);
    int local_offset_t  =  threadIdx.y*BLOCK_SIZE_SH + threadIdx.x; 
    ...
    if (iy < (height - 2))
    {
        if (ix < (width - 2))
        {
            float gx = gpu_applyFilter(&sh_block[local_offset_t], BLOCK_SIZE_SH, sobel_x, 3);
            float gy = gpu_applyFilter(&sh_block[local_offset_t], BLOCK_SIZE_SH, sobel_y, 3);
...
int main(int argc, char **argv)
{
    ...
    // Step 1: Convert to grayscale
    {
        ...
        store_result(11, elapsed[0], elapsed[1], bitmap.width, bitmap.height, image_out[0]);
    ...
    // Step 2: Apply a 3x3 Gaussian filter
    {
        ...
        gettimeofday(&t[1], NULL);
        store_result(22, elapsed[0], elapsed[1], bitmap.width, bitmap.height, image_out[1]);
        
        elapsed[0] = get_elapsed(t[0], t[1]);
        
        // Launch the GPU version
        gettimeofday(&t[0], NULL);
        gpu_gaussian_cached<<<grid, block>>>(bitmap.width, bitmap.height,
    ...
    // Step 3: Apply a Sobel filter
    {
        ...
        store_result(33, elapsed[0], elapsed[1], bitmap.width, bitmap.height, image_out[0]);
        gettimeofday(&t[1], NULL);
        ...
        // Launch the GPU version
        gettimeofday(&t[0], NULL);
        gpu_sobel_cached<<<grid, block>>>(bitmap.width, bitmap.height,
